...
class SymbolicRegressionDataset(Dataset):
    ...
    def __init__(self, npz_path: str, shuffle: bool = True):
        ...
        self.data = None  # 延迟加载，避免 pickle 问题
        self.num_samples = None
        self.indices = None
        self._load_metadata(shuffle)

    def _load_metadata(self, shuffle: bool):
        """加载元数据（样本数量）但不保留文件句柄。"""
        with np.load(self.npz_path, allow_pickle=True) as data:
            self.num_samples = len(data['input_dimensions'])
    ...
    def __setstate__(self, state):
        ...
        self._load_metadata(shuffle=False)
...
def make_batch_cpu(batch, vocab):
    """Create a training batch from dataset samples (no device transfer).

    Args:
        batch: List of samples from dataset (each is a dict from __getitem__)
        vocab: Vocabulary object for token IDs

    Returns:
        x_0: Unaligned base sequences (batch_size, seq_len)
        x_1: Unaligned target sequences (batch_size, seq_len)
        z_0: Aligned base sequences (batch_size, seq_len) - with gap tokens
        z_1: Aligned target sequences (batch_size, seq_len) - with gap tokens
    ...
    x_0 = torch.stack([F.pad(x, (0, x0_max_len - len(x)), value=vocab.pad_token) for x in x0_ids_list])
    x_1 = torch.stack([F.pad(x, (0, x1_max_len - len(x)), value=vocab.pad_token) for x in x1_ids_list])
    z_0 = torch.stack([F.pad(z, (0, z_max_len - len(z)), value=vocab.pad_token) for z in z0_ids_list])
    z_1 = torch.stack([F.pad(z, (0, z_max_len - len(z)), value=vocab.pad_token) for z in z1_ids_list])

    t = torch.rand(batch_size, 1)

    x_values_batch = torch.stack(x_values_list)
    y_target_batch = torch.stack(y_target_list)
